...
4. **`makeDefaultRspackConfig.ts`** — builds Cypress-specific rspack config: dev mode, HtmlRspackPlugin, CypressCTRspackPlugin. Note: it deliberately does NOT set `devtool` or `optimization` — `makeRspackConfig.ts` applies defaults (`devtool: eval-cheap-source-map`, `optimization: { sideEffects: false, splitChunks: false }`) only for the keys the user/framework config didn't set, so a user's choices are respected (this config is merged last and would otherwise clobber them)
